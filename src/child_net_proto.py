...
class resnet:
    ...
    def activation_function(self,inputs):
        ...
        for i in range(self.units):
            inputs = binary[self.activation[i*5+4]](unary[self.activation[i*5+2]](input_functions[self.activation[i*5+0]](inputs)),unary[self.activation[i*5+3]](input_functions[self.activation[i*5+1]](inputs)))
        return inputs

    def batch_norm_activation(self,inputs, is_training, data_format):
    ...
    def building_block(self,inputs, filters, is_training, projection_shortcut, strides,
      ...
      """
      shortcut = inputs
      inputs = self.batch_norm_activation(inputs, is_training, data_format)
    ...
    def bottleneck_block(self,inputs, filters, is_training, projection_shortcut,
      ...
      """
      shortcut = inputs
      inputs = batch_norm_activation(inputs, is_training, data_format)
      #inputs = batch_norm_relu(inputs, is_training, data_format)
    ...
    def cifar10_resnet_v2_generator(self,resnet_size, num_classes, data_format=None):
      ...
      def model(inputs, is_training):
        ...
        inputs = self.block_layer(
            inputs=inputs, filters=16, block_fn=self.building_block, blocks=num_blocks,
            strides=1, is_training=is_training, name='block_layer1',
            data_format=data_format)
        inputs = self.block_layer(
            inputs=inputs, filters=32, block_fn=self.building_block, blocks=num_blocks,
            strides=2, is_training=is_training, name='block_layer2',
            data_format=data_format)
        inputs = self.block_layer(
            inputs=inputs, filters=64, block_fn=self.building_block, blocks=num_blocks,
